...
Varyings DitherLitPassVertex(Attributes input)
{
	...
	output.remappedUV = float2(0, 1);
	output.remappedUV.x = map(output.uv.x, _SpriteUVRange.x, _SpriteUVRange.y, 0, 1);
	output.remappedUV.y = map(output.uv.y, _SpriteUVRange.z, _SpriteUVRange.w, 0, 1);

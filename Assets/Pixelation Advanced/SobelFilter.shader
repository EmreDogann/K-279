	...
	SubShader 
	{
		...
		Pass
		{
            ...
            half4 frag (Varyings input, out float depth : SV_Depth) : SV_Target 
            {
				...
				const float lum = dot(col, real3(0.2126729f, 0.7151522f, 0.0721750f));
				const float ditherLum = SAMPLE_TEXTURE2D(_NoiseMap, sampler_NoiseMap, input.uv * _Tiling).r;

				float ramp = (lum <= ditherLum) ? 0.1f : 1.0f;
				col = lerp(_BG, _FG, round(ramp));
